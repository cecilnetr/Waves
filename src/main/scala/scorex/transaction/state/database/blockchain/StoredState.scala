...
class StoredState(val storage: TheStorage, settings: ChainParameters)
  extends State with ScorexLogging {
  ...
  def totalAssetQuantity(assetId: AssetId): Long = AssetsExtendedState.getAssetQuantity(storage)(assetId)
